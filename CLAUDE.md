...
배포는 [DEPLOY.md](DEPLOY.md): 클라이언트 = GitHub Pages(main 브랜치 /docs), 서버 = NAS 의 Docker 컨테이너(Dockerfile/docker-compose.yml, DSM 역방향 프록시로 wss 노출). `docs/config.js` 의 `GAME_SERVER` 가 서버 주소.
...
- **레이팅 매칭** — `server/rating.py`(Elo 저장소, `DATA_DIR/ratings.json` 에 원자적 저장)와 `server.py` 의 대기열이 담당한다. 게임 로직이 아니므로 `Match` 는 손대지 않는다. 플레이어 식별은 브라우저 localStorage 의 UUID(`fightPlayerId`) — 계정/비밀번호가 없어 위조 가능하다는 전제. 매칭은 레이팅 순 정렬 후 인접한 둘을 붙이되 대기 시간에 비례해 허용 차를 넓힌다(`MATCH_RANGE`/`MATCH_WIDEN`). 결과 반영은 `Room._apply_rating` 이 매치당 한 번(`rated` 플래그, 재시작 시 해제).
