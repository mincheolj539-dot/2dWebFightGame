...
- **반격기(Counter)** — 지상에서 **방어(G) 홀드 중 공격(F)** 으로 발동(`Fighter._start_counter`). 히트박스 없는 반격 자세(`counter_timer`, 0.5초)에 진입하고, 그 안에 맞으면 `Match._check_hit` 이 피해를 무효화하고 공격자에게 `COUNTER_MULT`(1.5)배를 반사한다. 발동 중 금색 오라로 표시(`render_state`의 `counter` 필드).
